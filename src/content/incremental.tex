...
$M(q_1, G')$ will loose all its mappings, while $M(q_2, G')$ will find a lot of mappings
instead of none. 

\begin{figure}[h]
    \begin{subfigure}{.25\textwidth}
      \centering
      \begin {tikzpicture}[auto, node distance=2cm,thick,main node/.style={circle,draw}]
        \node[main node] (A) {$A$};
    ...
    \begin{subfigure}{.25\textwidth}
        \centering
        \begin {tikzpicture}[auto, node distance=2cm,main node/.style={circle,draw}]
          \node[main node] (A) {$1$};
          \node[main node] (B) [below=of A] {$2$};
          \node[main node] (C) [right =of B] {$3$};
          \node[main node] (D) [right =of A] {$4$};    
